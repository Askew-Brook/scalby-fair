...
<article {{ $attributes->class(['group']) }}>
    @if($article->featured_image)
        <a href="{{ $article->url() }}" tabindex="-1" aria-hidden="true"><x-responsive-image :asset="$article->featured_image" class="aspect-[4/3] w-full object-cover" /></a>
    @else
    ...
    <div class="pt-5">
        ...
        <h3 class="mt-2 font-serif text-2xl font-semibold tracking-tight text-balance text-hedge-900"><a href="{{ $article->url() }}" class="group-hover:text-barn-700">{{ $article->title }}</a></h3>
