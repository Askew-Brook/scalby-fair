...
<article {{ $attributes->class(['group grid h-full overflow-hidden border border-hedge-700/15 bg-cream-50 sm:grid-cols-[12rem_1fr]']) }}>
    @if($event->featured_image)
        <a href="{{ $event->url() }}" tabindex="-1" aria-hidden="true" class="min-h-52 overflow-hidden">
    ...
    <div class="p-6 sm:p-7">
        ...
        <h3 class="mt-2 font-serif text-2xl tracking-tight text-balance text-hedge-900 sm:text-3xl"><a href="{{ $event->url() }}" class="group-hover:text-barn-700">{{ $event->title }}</a></h3>
